// Agent runs commands over a browser session and returns text for an AI agent
// to read; it is the shared dispatcher for CLI, daemon (script) and MCP.
package agent
...
import (
	...
	// cdp is imported only for the *cdp.Client type; the protocol owner is
	// browser/dom, except the extension bridge, which returns a ready client.
...
// Close shuts down the connection and the browser, unless it was attached.
func (a *Agent) Close() {
	a.mu.Lock()
	defer a.mu.Unlock()
	if a.handle != nil {
...
// degraded reports whether the browser died or the connection dropped; without
// it a dead browser would leave the daemon answering errors forever.
...
func (a *Agent) ensure(ctx context.Context) (*browser.Session, error) {
	...
	case a.Engine == browser.EngineExt:
		// No browser is launched: the extension in Brave connects to the daemon.
...
// setAgent records who is driving; the extension renames the tab group, keeping
